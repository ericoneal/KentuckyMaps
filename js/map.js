...
const urlAGOLToken = 'https://pg53zgisx3.execute-api.us-east-1.amazonaws.com/default/eaoToken'



async function startMap() {

  await setAccessTokenGlobal();
  ...
  require([
    ...
    "esri/widgets/Search"], 
    (MapView, WebMap,esriId, esriConfig, Home, Track, LayerList, Measurement, Expand, Swipe, FeatureLayer, WebTileLayer, Search) => {
    ...
    esriConfig.request.proxyUrl = "https://eonealky.maps.arcgis.com/sharing/proxy";
    ...
    webmap.when(() => {
      console.log("WebMap is loaded");
        ...
        const stravaHeatmap = new WebTileLayer({
          urlTemplate: "https://heatmap-external-c.strava.com/tiles/sport_MountainBikeRide/green/{z}/{x}/{y}.png",
  ...
  function SendEmail(FromEmail, Message) {
    ...
    //Get the access token to use ESRI AGOL secured services
    function setAccessTokenGlobal() {
      return new Promise((resolve, reject) => {
        $.ajax({
            url: urlAGOLToken,
            method: "GET", // Adjust to POST if required
            dataType: "json",
            success: function(response) {
