...
require([
  ...
  const FLArches = new FeatureLayer({
    portalItem: {
      id: "9acb544225b5461ebc33a79698872d23"  
    },
    popupEnabled: true,
    popupTemplate: popupTemplate  
  });

  const FLOverlooks = new FeatureLayer({
    portalItem: {
      id: "85f90bdcdbb54b5098145ef406fbf7a9"  
    },
    popupEnabled: true,
    popupTemplate: popupTemplate  
  });
  ...
  view.when(() => {
        ...
        elevationLayer = webmap.allLayers.find(function (layer) {
          return layer.title === "Elevation";
        });

        if (!elevationLayer) {
          readout.innerHTML = "Elevation layer not found";
          return;
        }
  ...
  MakeImageryControls(view,toggleImagery);
  ...
  view.on("click", function (event) {
    updateElevationValue(event.mapPoint);
  });
...
$("#closeExpandButton").on("click", function() {
  ...
  searchWidget.on("select-result", function(event) {
    if(event.source.name == "Waterfall"){
      for(var i=0;i<=layerList.operationalItems.items.length -1;i++){
        if(layerList.operationalItems.items[i].title == "Waterfalls"){
          layerList.operationalItems.items[i].visible = true;
        }
      }
    ...
    view.goTo({
     ...
     if(event.source.name == "Arches"){
      for(var i=0;i<=layerList.operationalItems.items.length -1;i++){
        if(layerList.operationalItems.items[i].title == "Arches"){
          layerList.operationalItems.items[i].visible = true;
        }
      }
    ...
    view.goTo({
     ...
     if(event.source.name == "Overlooks"){
      for(var i=0;i<=layerList.operationalItems.items.length -1;i++){
        if(layerList.operationalItems.items[i].title == "Overlooks"){
          layerList.operationalItems.items[i].visible = true;
        }
      }
  ...
  function updateElevationValue(mapPoint) {
    esriRequest(elevationLayer.url + "/identify", {
      responseType: "json",
      query: {
        f: "json",
        ...
        geometry: JSON.stringify({
          x: mapPoint.x,
          y: mapPoint.y,
          spatialReference: mapPoint.spatialReference.toJSON()
        }),
        returnCatalogItems: false
      }
    }).then(function (response) {
      console.log(response.data);
      readout.innerHTML = "Elevation: " + response.data.value;
...
function MakeImageryControls(view,toggleImagery){
